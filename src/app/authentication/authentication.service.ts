...
export class AuthenticationService {
  ...
  setPassword(data: any) {
    let header: any = {
      headers: { verifiedAccount: localStorage.getItem("verifiedAccount") },
    };
    return this.http.post(
      this.baseURL + "/v1/seller/setpassword/seller",
      data,
      header
    );
  }
  ...
  get_state(country: any) {
    return this.http.get(
      this.baseURL + `/v2/agora/get/state?county=${country}`
    );
  }
  get_city(country: any, state: any) {
    return this.http.get(
      this.baseURL + `/v2/agora/get/city?county=${country}&state=${state}`
    );
  }
